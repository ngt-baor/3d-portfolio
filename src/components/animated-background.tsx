...
const KeyboardScene = ({ maxDpr }: { maxDpr: number }) => {
  ...
  const canUseSkillVariables = () => {
    if (!splineApp) return false;
    try {
      return (
        splineApp.getVariable("heading") !== undefined &&
        splineApp.getVariable("desc") !== undefined
      );
    } catch {
      return false;
    }
  ...
  const clearSkillVariables = () => {
    if (!splineApp || !canUseSkillVariables()) return;
    splineApp.setVariable("heading", "");
    splineApp.setVariable("desc", "");
  };

  const setSkillVariables = (skill: Skill) => {
    if (!splineApp || !canUseSkillVariables()) return;
  ...
  useEffect(() => {
    ...
    setSkillVariables(selectedSkill);
    ...
    <>
      <Suspense fallback={<div>Loading...</div>}>
        <Spline
          ...
          scene="/assets/skills_keyboard.spline?v=20260624d"
        />
      </Suspense>
      {activeSection === "skills" && selectedSkill && !canUseSkillVariables() && (
        <div className="pointer-events-none fixed left-[8vw] top-[30vh] z-[3] max-w-[22rem] text-left text-white drop-shadow-[0_6px_0_rgba(0,0,0,0.35)] dark:text-white">
          <div className="font-display text-4xl font-black leading-none md:text-6xl">
            {selectedSkill.label}
          </div>
          <div className="mt-3 max-w-[18rem] text-xl font-semibold leading-tight text-white/85 md:text-2xl">
